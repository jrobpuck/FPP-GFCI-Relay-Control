...
def current_status(cfg):
    """Raw GET /api/fppd/status response. This is a real-time signal
    independent of /api/schedule - it catches anything FPP is playing
    regardless of whether a Scheduler entry predicted it (a manually-started
    playlist, one triggered by an FPP Command/Event, a test run, ...).
    Schedule-based prediction is still what lets arm_lead_seconds/
    disarm_lag_seconds do anything, since there is no "before" for a start
    FPP didn't know about in advance.

    Uses /api/fppd/status specifically (not /api/player/status, which this
    used at first): only /api/fppd/status was actually traceable to real
    code (PlayerResource::GetCurrentStatus() -> GetCurrentFPPDStatus() ->
    Playlist::GetCurrentStatus()) - see NOTES.md. /api/player/status's
    shape was inferred by analogy from openapi.json's description text
